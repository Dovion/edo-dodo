...
> **Примечание:** в текущей версии реализованы загрузка актов (в т.ч. из 1С XML), отправка в СБИС (`СБИС.ЗаписатьДокумент`), ручная смена статусов и очередь бухгалтерии. Периодический опрос `СБИС.ПрочитатьДокумент` (планировщик) и автоматическая обратная синхронизация в 1С отражены на схеме как целевые шаги процесса.

### Этапы (основной сценарий)

1. **1С → edo-dodo** — акты выгружаются из 1С (XML и вложения) и попадают в реестр со статусом **«Загружено»**.
2. **Отправка в СБИС** — пользователь отправляет акт в ЭДО ([`СБИС.ЗаписатьДокумент`](https://saby.ru/help/integration/api/all_methods/doc)); статус **«Отправлено в СБИС»**, сохраняется `sabySendId`.
3. **Контрагент** — документ уходит контрагенту через СБИС; при необходимости фиксируется **«Отправлено Контрагенту»**.
4. **Планировщик** — по расписанию для актов в СБИС вызывается [`СБИС.ПрочитатьДокумент`](https://saby.ru/help/integration/api/all_methods/read_doc) (поле `Состояние` / этапы подписания). Если акт **подписан** — выгрузка подписанной версии и статус **«Получен подписанный»**.
5. **Бухгалтерия** — бухгалтер сверяет акт в очереди (**«В работе бухгалтерии»** / подписанные) и переводит в **«Закрыт»**.
6. **edo-dodo → 1С** — обратная синхронизация закрытых актов в 1С.
